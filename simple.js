...
const ktod = "6285649064565@s.whatsapp.net"

const htod = "6285649064565@s.whatsapp.net"
...
module.exports = simple = async (simple, mek, _welkom) => {
	try {
		...
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6285804325802@s.whatsapp.net`]
...
//━━━━━━━━━━━━━━━[ CONNECTION 1 ]━━━━━━━━━━━━━━━━━//
		...
		faketeks = '®♡ۣۜۜ፝͜͜͡͡✿➣フェアラス' //Ganti Aja Nama Luh
		const isUrl = (url) => {
...
//>>>>>>>>>>>>>>>>>>>>>[ BUTTON TAMBAHAN ]<<<<<<<<<<<<<<<<<<<<<<<\\
        ...
        const jelek = { "title": `Hallo ${pushname}`,"body": `hy`, "mediaType": "2", "mediaUrl": "https://chat.whatsapp.com/CONEEVXvT8JCn4ZqYJVims?feature=share", "thumbnail": fs.readFileSync('./poto/YENI.jpg')}
		...
		message: {
			"productMessage": {
				"product": {
					...
					"description": `♡ۣۜۜ፝͜͜͡͡✿➣フェアラス`, 
					"currencyCode": "Rp",
					"priceAmount1000": "1.000.000.000.000",
...
Ppurl = 'https://telegra.ph/file/5f874b717ba953874a123.jpg'
...
├ ツ Penyusun : *Fairuzz*
├ ツ Terbit : *15-01-2022*
├ ツ Tempat : *Kota Pasuruan, Jawa Timur*
└❏
┌────“ 💭 „────
...
if (!isRegistered) return sendButRegis(from, daftar1, daftar2, daftar3, { quoted: fgif})
   ...
   sendButImage(from, pll, `FairuzBotz© 1945`, cewe, [                      
          {
...
[`sholawat1`,`sholawat2`,`sholawat3`,`sholawat4`,`sholawat5`,`sholawat6`,`sholawat7`,`sholawat8`,`sholawat9`,] 
               ...
               startnum = 0
               for (let x of listmenu) {
               const yy = {title: '♡ۣۜۜ፝͜͜͡͡✿➣フェアラス ' + nombor++,
                    rows: [
                       {
                        title: `${listmenuu[startnum++]}`,
                        description: ``,
...
𝐃𝐨𝐧'𝐭 𝐒𝐩𝐚𝐦`
oy =`
*_Jasa Sewa Bot By : ©♡ۣۜۜ፝͜͜͡͡✿➣Fairuzz_
*>>>>>>>> *LIST HARGA* >>>>>>>>*
[🔥] . _Fitur Bot Banyak And Ada Catalog Menu_

🐣. _*2Minggu : Rp. 5.000 [ Promo ]*_
🐣. _*1Bulan : Rp. 10.000 [ Promo ]*_
🐣. _*Manent : Rp. 20.000 [ Promo ]*_

 _Jika Anda Minat Bisa Chat Nomer Ini_
_*🪀 https://wa.me/6285649064565*_
_Note Jangan Spam_
...
costum(buffer, sticker, Verived, `Jangan Lupa Join Grup Owner:)`)
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
buffer = fs.readFileSync(ran)
costum(buffer, image, Verived, `©_♡ۣۜۜ፝͜͜͡͡✿➣フェアラス_ *V : 2.889H*`)
fs.unlinkSync(ran)
})
break
case 'tomp3':
...
fs.unlinkSync(anum)
buffer = fs.readFileSync(ran)
simple.sendMessage(from, buffer, video, { quoted: ftrol, caption: 'Done... Jangan Lupa Join Grup Owner:)' })
fs.unlinkSync(ran)
})
break
...
but = [
        ...
        sendButImage(from, kimak, "© Fairuzz | ᴮᴼᵀ", img, but)
break
...
simple.updatePresence(from, Presence.recording)
Gtau = 'https://telegra.ph/file/5f874b717ba953874a123.jpg'
...
reply('```W A I T I N G```')
                nama = ' 🔥Fairuzz🔥 '
       ownerJid = "6285649064565@s.whatsapp.net"
                nomor = 1
                itod = `${sender}`
...
🐼  _Jam : ${time}_
🐼  _Mode : START-MODE_
🐼  _*Total User : User Aktif 1.868.924.062*_
...
tescok = `❏  _*Runtime : ${runtime(process.uptime())}*_
  ...
  「 BAILEYS SELF-BOT」 `
Sendbutdocument(from, tescok, "© ♡ۣۜۜ፝͜͜͡͡✿➣フェアラス*", fs.readFileSync('./poto/menu.pdf'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./poto/image/thumbnaildokumen.jpg'), filename:`Fairuzz.pdf`, pageCount: 9999999 },[{ buttonId: `${prefix}menu`, buttonText: { displayText: 'SIP,KAK😁' }, type: 1 }], {quoted:fvid, contextInfo: { mentionedJid: [dtod,otod,stod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} - ${pushname}\nJoin Grup Owner :`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://chat.whatsapp.com/CONEEVXvT8JCn4ZqYJVims`}}})
break
...
teks = `
💾Sc : *https://github.com/FairusAnjay.io*
_Spek Sc_ :
1. *Antilink*
2. *Welcome Button*
3. *Antivirtex*
...
but = [
        ...
        sendButImage(from, teks, '©Created : Fairuzz', foto, but,)
break
case 'tourl':
